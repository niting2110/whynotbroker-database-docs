...
/**
 * CompleteDocumenter
 * - Safer DB connection handling
 * - Avoids globally disabling TLS
 * - Uses pg-format for identifier interpolation
 * - Simpler, robust row counts
 *
 * Environment variables used:
 * - DATABASE_URL (required)
 * - OUTPUT_DIR (optional) default: project root (one level up from this file)
 * - DB_SSL (optional) "true" to force ssl: { rejectUnauthorized: false } (use carefully)
 * - ALLOW_INSECURE_TLS (optional) "true" to allow setting NODE_TLS_REJECT_UNAUTHORIZED=0 (NOT recommended)
 */
...
const format = require('pg-format'); // add to package.json: "pg-format"
...
class CompleteDocumenter {
  constructor() {
    ...
    // Do NOT disable TLS globally; allow explicit opt-in (discouraged)
    if (process.env.ALLOW_INSECURE_TLS === 'true') {
      console.warn('⚠️ ALLOW_INSECURE_TLS=true — Node will accept unauthorized TLS certificates. This is unsafe for production.');
      process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';
    }

    // Decide SSL config
    // - If DB_SSL=true, set ssl with rejectUnauthorized false (for Supabase-like services)
    // - Else don't pass ssl (pg will parse sslmode from the connection string if present)
    ...
    this.outputDir = path.resolve(process.env.OUTPUT_DIR || path.join(__dirname, '..'));
  ...
  async generateAll() {
    ...
    try {
      ...
      for (const t of tables) {
        ...
        // Get constraints (join to pg_constraint to obtain definitions)
        // Use pg_catalog to get constraint details
        const constraintsQ = await client.query(
          `
          SELECT
            tc.constraint_name,
            tc.constraint_type,
        ...
        // Get row count safely using pg-format for identifier interpolation
        let rowCount = 0;
        try {
        ...
        tableDetails[tableName] = {
          ...
          constraints: constraintsQ.rows,
          row_count: rowCount,
          column_count: parseInt(t.column_count, 10)
        };
      }
      ...
      await this.generateCheatsheets(tableDetails.properties || tableDetails['properties']);
      ...
      // Don't call process.exit immediately — ensure pool is closed in finally
      throw error;
    } finally {
      try {
        if (client) {
        ...
        console.warn('Error releasing client:', e.message);
      }
  ...
  async generateReadme(tables, tableDetails) {
    ...
    const tableRows = tables.map(t => {
      const rows = (tableDetails[t.table_name] && typeof tableDetails[t.table_name].row_count === 'number')
        ? `~${tableDetails[t.table_name].row_count.toLocaleString()}`
        : '~0';
      return `| \`${t.table_name}\` | ${t.column_count} | ${rows} |`;
    }).join('\n');
...
`;
    ...
    for (const t of tables) {
      ...
      const details = tableDetails[name] || { columns: [], constraints: [], row_count: 0, column_count: 0 };
...
const pool = new Pool({
  ...
  ssl: { rejectUnauthorized: false } // only if your env requires it
});
\`\`\`
...
`;
    ...
    await this.safeWrite(path.join('cheatsheets', 'insert-property.md'), insertContent);
  }
...
## Key Tables
${tables.map(t => `- \`${t.table_name}\`: ${t.column_count} columns, ~${tableDetails[t.table_name]?.row_count || 0} rows`).join('\n')}
